...
//双向绑定---为什么不用ref呢，因为我们要实现不仅内部可以控制这个弹框的展示，其他地方也可以
const modelValue = defineModel<boolean>('modelValue',{required:true})
//scoket--这里是监听那个触发的paymentSuccess事件
const { getSocket } = useSocket()
const userStore = useUserStore()
//modelValue为true则会弹出这个支付框所以这时候我们就能监听到这个paymentSuccess事件了
...
const onConfirm = async ()=>{
    ...
    console.log(body);
    // 发送支付宝请求
    const response = await createPay(body)
    if(response.code === 200){
        isPay.value = true //设置状态为支付中
        window.open(response.data.payUrl,"_blank") //浏览器新打开一个窗口，支付链接
        timeExpire.value = response.data.timeExpire
    }else{
